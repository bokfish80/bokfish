...
// 기기 로컬 날짜(YYYY-MM-DD) 가져오기 (UTC 오류 방지)
const getLocalDate = () => {
  const now = new Date();
  return now.toLocaleDateString('en-CA'); // YYYY-MM-DD 형식 보장
};

const getDeviceId = () => {
  let id = localStorage.getItem('seokpo_fixed_id_v8');
  if (!id) {
    id = 'dev_' + Math.random().toString(36).substring(2, 11);
    localStorage.setItem('seokpo_fixed_id_v8', id);
  }
  return id;
};
...
const BASE_KEY = "seokpo_v8_final"; 

const App: React.FC = () => {
  const STORAGE_KEY_PREFIX = 'sg_v8_';
  ...
  const mergeData = useCallback((remote: any) => {
    ...
    if (remote.timestamp > lastUpdateAtRef.current) {
      ...
      // 학생 명단 병합 (중복 제거 및 최신화)
      if (remote.students && remote.students.length > 0) {
        setStudents(prev => {
          // 명단이 더 큰 쪽을 우선하되 로컬에서 방금 추가한 데이터가 사라지지 않게 처리
          return remote.students.length >= prev.length ? remote.students : prev;
        });
      }
      
      if (remote.violations) setViolations(remote.violations);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => pull(false), 5000); // 5초 주기로 안정화
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center">
        <div className="space-y-6 animate-pulse">
          ...
          <h2 className="text-2xl font-black text-white">데이터 동기화 엔진 가동 중...</h2>
          <p className="text-slate-500 font-bold">잠시만 기다려주세요.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 text-center">
        <div className="max-w-md w-full space-y-10">
          ...
          <div>
            <h1 className="text-3xl font-black text-white tracking-tight">석포여자중학교<br/><span className="text-pink-500">지각관리시스템</span></h1>
          </div>
          <div className="bg-slate-900/50 p-10 rounded-[3rem] border border-white/5 space-y-8 shadow-3xl">
            <input 
              ...
              maxLength={4}
              placeholder="암호입력"
              className="w-full bg-slate-800/50 border-none rounded-3xl px-6 py-6 text-4xl font-black text-pink-500 placeholder:text-slate-700 text-center tracking-[0.5em]"
    ...
    <div className="min-h-screen flex flex-col bg-[#f8fafc]">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4 cursor-pointer" onClick={() => {setCurrentYear(null); setCurrentClass(null); setActiveTab('list');}}>
              ...
              <div>
                <h1 className="text-xl font-black text-slate-900">석포여중 지각관리</h1>
                <div className={`flex items-center gap-1.5 mt-1 text-[9px] font-black ${syncStatus === 'error' ? 'text-red-500' : 'text-slate-500'}`}>
                  {syncStatus === 'syncing' ? <RefreshCw size={10} className="animate-spin" /> : <Wifi size={10} />}
                  {lastSyncTime ? `${lastSyncTime} 동기화됨` : '연결 중...'}
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => push(true)} className={`p-3 rounded-2xl transition-all ${isLocalChanged ? 'bg-pink-600 text-white shadow-lg' : 'bg-white border border-slate-200 text-slate-400'}`}>
                <Save size={18} />
              </button>
              <button onClick={() => { if(confirm('로그아웃 하시겠습니까?')){ localStorage.removeItem(STORAGE_KEY_PREFIX + 'auth'); window.location.reload(); } }} className="bg-slate-950 text-white p-3.5 rounded-2xl"><Unlock size={18} /></button>
            </div>
          </div>
          
          <div className="flex flex-col md:flex-row gap-3 mt-5">
            <div className="relative flex items-center bg-white rounded-2xl px-5 py-3.5 border border-slate-200 shadow-sm">
              <Calendar size={18} className="text-pink-500 mr-3" />
              <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)} className="bg-transparent text-sm font-black focus:outline-none text-slate-700" />
            </div>
            <div className="relative flex-1">
              <Search size={20} className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400" />
              <input type="text" placeholder="학생 검색..." className="w-full bg-white border border-slate-200 rounded-2xl pl-14 pr-6 py-3.5 text-sm font-bold focus:outline-none shadow-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
          </div>
        </div>
      ...
      <div className="bg-slate-900 text-white">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3.5 flex items-center justify-between">
          <div className="flex gap-3">
            <span className="bg-yellow-400 text-slate-900 px-3 py-1 rounded-full text-[10px] font-black">지각 {statsCount.late}</span>
            <span className="bg-red-500 text-white px-3 py-1 rounded-full text-[10px] font-black">결석 {statsCount.absent}</span>
            <span className="bg-white/10 text-white px-3 py-1 rounded-full text-[10px] font-black">정상 {statsCount.total - (statsCount.late + statsCount.absent)}</span>
          </div>
          ...
          <div className="space-y-8">
            {(currentYear === null && !searchQuery) ? (
              ...
              <div className="space-y-6">
                <button onClick={() => setCurrentYear(null)} className="flex items-center gap-2 text-slate-500 font-black hover:text-pink-600"><ChevronLeft size={20} /> 학년 선택</button>
              ...
              <div className="space-y-6">
                {!searchQuery && <button onClick={() => setCurrentClass(null)} className="flex items-center gap-2 text-slate-500 font-black hover:text-pink-600"><ChevronLeft size={20} /> 반 선택</button>}
      ...
      <footer className="py-8 text-center border-t border-slate-100 bg-white">
         <div className="flex items-center justify-center gap-3 text-[10px] font-black text-slate-400 uppercase tracking-widest">
           <span>석포여자중학교 지각관리 v8.0 (Bulletproof)</span>
           <span className="flex items-center gap-1.5">김용섭 제작 <Heart size={10} className="text-pink-500 fill-pink-500" /></span>
         </div>
      </footer>
    </div>
  );
